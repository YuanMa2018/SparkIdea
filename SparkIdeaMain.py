...
# IP are from server
def StartRPConServerEnd(Proxy,KeyWord, NewsSource):
    print("-----START RPC ON SERVER -----")
    s = ServerProxy(Proxy)
    s.StartMonitor(KeyWord, NewsSource)
...
def RealTimeRenderDataFromDB(isLogin,KeyWord, NewsSource):
    if isLogin == '1':
        ...
        t2 = threading.Thread(target=StartRPConServerEnd, args=("http://192.168.2.152:49000",KeyWord, NewsSource))
        t3 = threading.Thread(target=StartRPConServerEnd, args=("http://192.168.2.153:49000",KeyWord, NewsSource))
        t2.start()
        t3.start()
